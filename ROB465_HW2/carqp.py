...
def linearize_dynamics_numerically(x_r, u_r, h, true_dynamics):
    ...
    for i in range(m):
        for j in range(n):
            ...
            xu_r_copy = xu_r + np.zeros_like(xu_r)  # Create a copy of xu_r
            xu_r_copy[j] += h  # Perturb the j-th element
            Jacobian[i, j] = (f(xu_r_copy)[i] - f_r[i]) / h
...
def optimize_single_action(goal_state, current_state, reference_control, A, B, speed_limit, turn_limit):
    ...
    A_ = np.array([[-1, 0], [1, 0], [0, -1], [0, 1]])
    b = np.array([-speed_limit[0], speed_limit[1], -turn_limit[0], turn_limit[1]])

    control = cvx.Variable(shape=(B.shape[1],))  # Define the control variable


    P = B.T @ B
    obj = goal_state - current_state + B @ reference_control # Calculate P and the objective function components

    # Define the objective function
    objective = cvx.Minimize(cvx.quad_form(control, P) - 2 * obj.T @ B @ control)

    # Define constraints
    constraints = [A_ @ control <= b]
